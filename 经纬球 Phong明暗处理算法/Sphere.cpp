...
void CSphere::DrawFacet(CDC* pDC, CP3* P)//绘制小面
{
	...
	if (DotProduct(ViewVector, FacetNormal) >= 0)//背面剔除
	{
		for (int nPoint = 0;nPoint < 4;nPoint++)
		{
			...
			Normal4[nPoint] = CVector3(P[nPoint]);
		}
		pFill->SetPoint(Point4[0], Point4[1], Point4[2], Normal4[0], Normal4[1], Normal4[2]);//下三角形
		pFill->PhongShader(pDC, projection.EyePoint, pLight, pMaterial);
		pFill->SetPoint(Point4[0], Point4[2], Point4[3], Normal4[0], Normal4[2], Normal4[3]);//上三角形
		pFill->PhongShader(pDC, projection.EyePoint, pLight, pMaterial);
	}
	delete pFill;
}
